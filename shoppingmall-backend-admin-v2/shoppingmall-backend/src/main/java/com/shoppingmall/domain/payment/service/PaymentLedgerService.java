...
/**
 * 토스 결제 승인 과정에서 DB 원장(payments/orders)만 다루는 서비스.
 *
 * <p><b>왜 PaymentService 에서 분리했는가</b><br>
 * 토스 승인은 외부 HTTP 호출이라 수 초가 걸릴 수 있다. 이걸 트랜잭션 안에서 부르면
 * (1) 그동안 DB 커넥션을 붙잡고 있게 되고,
 * (2) 승인 후 예외가 나면 롤백되면서 "토스는 승인했는데 우리 DB엔 흔적이 없는" 상태가 된다.
 * 그래서 DB 구간(READY 저장 / PAID / FAILED)을 여기에 몰아넣고 각각 독립 트랜잭션으로 커밋하며,
 * 외부 호출은 트랜잭션 밖(PaymentService.confirmPayment)에서 한다.
 *
 * <p>REQUIRES_NEW 를 쓰되 <b>반드시 다른 빈에서 호출해야 한다.</b> 같은 클래스 안에서 호출하면
 * Spring 프록시를 거치지 않아(self-invocation) 전파 속성이 통째로 무시된다.
 * 이 클래스를 PaymentService 안의 private 메서드로 합치면 안 되는 이유가 이것이다.
 */
@Slf4j
@Service
...
public class PaymentLedgerService {
    ...
    /** 승인 요청 직전 상태. 토스에 보낼 금액은 클라이언트 값이 아니라 여기서 확정한 서버 값을 쓴다. */
    ...
    public Prepared prepare(Long userId, PaymentConfirmRequest request) {
        ...
        // 4. 금액 위변조 검사 - 토스를 부르기 전에 차단한다.
        //    실제 승인 금액은 토스 응답의 totalAmount 와 한 번 더 대조한다(PaymentService).
        int expectedAmount = order.getFinalPaymentAmount();
        if (expectedAmount != request.getAmount()) {
            log.warn("[TOSS] 결제 금액 불일치. orderNumber={}, 서버={}, 요청={}",
                    order.getOrderNumber(), expectedAmount, request.getAmount());
            throw new CustomException(ErrorCode.PAYMENT_AMOUNT_MISMATCH);
        }

        // 5. 이 paymentKey 를 다른 주문이 이미 선점했는지 확인 (pg_transaction_id 는 UNIQUE)
        ...
        // 6. 결제 원장 READY 저장.
        //    payments.order_id 에 UNIQUE 인덱스가 있어 주문당 한 행뿐이므로,
        ...
        // flush 를 강제해 UNIQUE 위반 같은 제약 오류가 토스 호출 전에 드러나게 한다
     ...
     * (재시도 시 이 행을 prepareRetry 로 재사용한다)
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
